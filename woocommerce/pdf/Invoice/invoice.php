...
<h1 class="document-type-label">
	<?php echo pll__('Рахунок-фактура'); ?> № <?php echo $this->order_number(); ?>
	<?php //if ( $this->has_header_logo() ) $this->title(); ?>
</h1>

<?php do_action( 'wpo_wcpdf_after_document_label', $this->get_type(), $this->order ); ?>
<h4><strong><?php echo pll__('Отримувач'); ?>: </strong></h4>
	...
	<tr>
		<td class="address billing-address">
			...
			<?php if ( isset( $this->settings['display_email'] ) ) : ?>
				<div class="billing-email"><?php echo $this->billing_email(); ?></div>
			<?php endif; ?>
			<?php if ( isset( $this->settings['display_phone'] ) ) : ?>
				<div class="billing-phone"><?php echo $this->billing_phone(); ?></div>
			<?php endif; ?>
			</br>
			<?php echo $this->order->get_user()->data->user_email; ?>
			</br>
			<?php echo get_user_meta($this->order->get_user()->data->ID,'billing_phone',true); ?>
		</td>
		<td class="address shipping-address">
			<?php if ( $this->show_shipping_address() ) : ?>
				...
				<?php if ( isset( $this->settings['display_phone'] ) ) : ?>
					<div class="shipping-phone"><?php echo $this->shipping_phone(); ?></div>
				<?php endif; ?>
			<?php endif; ?>
		</td>
		<td class="order-data">
	...
	<tbody>
		<?php foreach ( $this->get_order_items() as $item_id => $item ) : ?>
			<tr class="<?php echo apply_filters( 'wpo_wcpdf_item_row_class', 'item-'.$item_id, esc_attr( $this->get_type() ), $this->order, $item_id ); ?>">
				...
				<td class="price"><?php echo str_replace('&#8372;','', $item['order_price'] ) . ' грн.' ; ?></td>
			</tr>
		<?php endforeach; ?>
	</tbody>
	<tfoot>
							...
							<tr class="<?php echo esc_attr( $key ); ?>">
								...
								<?php if ( $total['label'] == 'Доставка' ) : ?>
									...
									<?php if ( get_post_meta( $this->order->get_id(), 'delivery_method' )[0] == 'courier' ) : ?>
										<td class="price"><span class="totals-price"><?php echo str_replace('&#8372;','', $this->order->get_shipping_total() ) . '.00 грн.';?></span></td>
									<?php endif; ?>
								<?php else : ?>
									<td class="price"><span class="totals-price"><?php echo str_replace('&#8372;','', $total['value'] ) . ' грн.'; ?></span></td>
								<?php endif; ?>
							</tr>
						<?php endforeach; ?>
					</tfoot>
